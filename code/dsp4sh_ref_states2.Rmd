...
```{r Figure 1A - dual-axis climate figure}
# Compile data
clim <- soc_pedon %>%
...
soc_nolett_grid <- plot_grid(soc30_boxplot_nolett + 
...
ggsave(here("figs", "ref_states_figs","fig2_dsp4sh_soc_stocks.png"), width=11, height=10, units="in", dpi=400)
```
...
```{r Figure S2 - comparison of ESM to fixed depth results}
...
```{r Table S1 - table of fixed depth vs ESM stock results}
...
```{r Fig 3 - ribbon plot of SOC stocks with depth, echo=FALSE}
...
plot_grid(pred30, pred100,
          ncol=1, labels=c("A", "B"))
ggsave(here("figs", "ref_states_figs","lmer_results.png"), width=9, height=9, units="in", dpi=400)
```
...
ggsave(here("figs", "ref_states_figs","fig4_shape_scores.png"), width=10, height=10, units="in", dpi=400)
```
...
```{r Figure 5 - shape vs SOC for eco states}
